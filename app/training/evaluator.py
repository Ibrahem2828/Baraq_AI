...
def summarize_evaluations(rows: Iterable[dict[str, Any]]) -> EvaluationMetrics:
    ...
    exact_values = [
        item.get("exact_match") for item in items if item.get("exact_match") is not None
    ]
    quality_values = [
        float(item["quality_score"]) for item in items if item.get("quality_score") is not None
    ]
    return EvaluationMetrics(
        ...
        average_quality_score=sum(quality_values) / len(quality_values) if quality_values else None,
