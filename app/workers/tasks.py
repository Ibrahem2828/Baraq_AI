...
from app.workers.async_runner import run as run_async
...
def dispatch_job_outbox() -> int:
    ...
    return run_async(OutboxDispatcher().dispatch_pending(_enqueue_outbox_event))
...
def recover_stale_ai_jobs() -> tuple[int, int]:
    return run_async(_recover_stale_jobs())
