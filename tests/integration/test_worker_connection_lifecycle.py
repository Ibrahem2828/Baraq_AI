...
@pytest.mark.asyncio
async def test_repeated_worker_task_cycles_leave_no_idle_in_transaction_connections() -> None:
    from app.db.session import AsyncSessionLocal, dispose_engine, get_engine

    # Bootstrap: create the schema once, through a throwaway direct engine
    # (not the module-under-test's cached one -- that would pre-empt the
    # very thing this test is about to exercise for real).
    ...
    async def one_task_cycle(marker: int) -> None:
        ...
        async with AsyncSessionLocal() as session:
            await session.execute(text("SELECT :marker"), {"marker": marker})
    ...
    # Inspect from a fresh, separate connection/session -- not through the
    # engine under test, so the inspection query itself cannot be mistaken
    # for one of the connections it is counting.
    ...
    try:
        async with inspector_engine.connect() as connection:
            ...
            backends = [dict(row._mapping) for row in rows]
    ...
    # worker_process_shutdown does for a real worker) must not itself throw
    # -- the whole class of bug this test exists for was exactly a close
    # attempt failing and logging "Exception terminating connection".
    await dispose_engine()


@pytest.mark.asyncio
async def test_dispose_engine_after_many_cycles_closes_every_pooled_connection() -> None:
    ...
    try:
        async with inspector_engine.connect() as connection:
            rows = await connection.execute(
                text("SELECT count(*) FROM pg_stat_activity WHERE application_name = :app_name"),
                {"app_name": _application_name()},
            )
            remaining = rows.scalar_one()
        assert remaining == 0, (
